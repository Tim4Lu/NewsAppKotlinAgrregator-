...
TAG="v156"
OUTPUT_FILE="/sdcard/Нова папка/full_code_export_v156.txt"

TEMP_DIR=$(mktemp -d)

# Витягуємо файли тегу v156 у тимчасову папку
git archive "$TAG" | tar -x -C "$TEMP_DIR" 2>/dev/null

if [ $? -ne 0 ]; then
    echo "Спроба $TAG не вдалася, пробуємо 156..."
    TAG="156"
    git archive "$TAG" | tar -x -C "$TEMP_DIR" 2>/dev/null
fi

if [ ! -d "$TEMP_DIR/app" ]; then
    echo "Помилка: тег або коміт $TAG не знайдено! Перевірте список через 'git tag'."
    rm -rf "$TEMP_DIR"
    exit 1
fi
