...
object AiRewriter {
    ...
    suspend fun translateFullArticle(newsItem: NewsItem): String? {
        ...
        val prompt = "Ти — провідний український науковий оглядач та перекладач. Зроби глибокий, адаптований та якісний переклад усієї статті українською мовою. Адаптуй подачу матеріалу так, щоб він звучав природно для української аудиторії, зберігши всі наукові факти, терміни та цифри. Нічого не скорочуй.\n\nЗаголовок: ${newsItem.originalTitle.ifEmpty { newsItem.title }}\nТекст: $textToTranslate"
    ...
    suspend fun processAllNewsWithAi(newsList: List<NewsItem>, context: Context? = null, onItemProcessed: (NewsItem) -> Unit) {
        ...
        try {
            ...
            for (item in newsToProcess) {
                ...
                try {
                    val prompt = "Зроби пост для Telegram українською за такою структурою:\n1. Яскравий заголовок (без емодзі-ракети на початку дублювання).\n2. Основний опис суті новини (2 речення, що пояснюють головну подію, БЕЗ переліку фактів нижче).\n3. Рівно 3 унікальних цікавих факти булітами (•), які НЕ дублюють інформацію з основного опису вище (розкривають деталі, контекст, цифри або наслідки).\nБез вступів, без \"Ось переклад\", без **. Джерело не пиши.\n\nЗаголовок: ${item.title.replace("\"", "'").replace("\n", " ").replace("🚀", "")}\nТекст: ${item.description.replace("\"", "'").replace("\n", " ")}"
                    var translatedText: String? = null
                    var attempts = 0
